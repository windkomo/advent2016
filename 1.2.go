...
func main() {
	...
	for _, instruction := range instructions {
		...
		for i := 0; i < steps; i++ {
			walker.walk()
			hasVisited = SliceIndex(len(walker.visitedCells), func(i int) bool {
				return walker.visitedCells[i].x == walker.position.x && walker.visitedCells[i].y == walker.position.y
			}) > -1

			if hasVisited {
				break
			} else {
				walker.visitedCells = append(walker.visitedCells, Position{walker.position.x, walker.position.y})
			}
		}
